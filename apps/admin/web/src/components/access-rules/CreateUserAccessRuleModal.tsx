...
import {
  useCreateUserAccessRule,
  type CreateUserAccessRuleRequest,
...
interface CreateUserAccessRuleModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export const CreateUserAccessRuleModal = ({
  isOpen,
  onClose,
  applicationId,
  applicationName,
  onSuccess,
}: CreateUserAccessRuleModalProps) => {
  const [ruleType, setRuleType] = useState<RuleType>("ACCEPT");
  const [subjectType, setSubjectType] = useState<SubjectType>("USER");
  const [subjectId, setSubjectId] = useState("");
  const [error, setError] = useState<string | null>(null);

  const { createUserAccessRule, isLoading } = useCreateUserAccessRule();
  ...
  const handleSubmit = () => {
    ...
    const request: CreateUserAccessRuleRequest = {
      applicationId,
      ruleType,
      subjectType,
          ...
          <VStack gap={4} align="stretch">
            <Box>
              <Text fontSize="sm" color="gray.600" mb={3}>
                Create a new access rule for <strong>{applicationName}</strong>
              </Text>
            </Box>
